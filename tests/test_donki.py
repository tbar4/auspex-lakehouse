...
def test_resource_yields_list_items(monkeypatch):
    ...
    res = _donki_resource("nasa_donki_coronal_mass_ejections", "CME", "activityID")
...
def test_resource_tolerates_non_list_body(monkeypatch):
    ...
    res = _donki_resource("nasa_donki_geomagnetic_storms", "GST", "gstID")
...
def test_resource_builds_request_with_extra_params(monkeypatch):
    ...
    res = _donki_resource("nasa_donki_notifications", "notifications", "messageID", {"type": "all"})
...
def test_registry_has_11_unique_endpoints():
    ...
    notif = next(e for e in DONKI_ENDPOINTS if e[0] == "nasa_donki_notifications")
